...
Pending registrations appear in the **Approval Queue** accessible from the Members page header (Pastors and Admins only).
...
- `DepartmentLeader` scoped to a specific department → unlocks department roster and follow-ups
- `BranchAdmin` scoped to a branch → grants Pastor-level access for that branch
