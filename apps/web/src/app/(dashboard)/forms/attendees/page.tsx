...
import { useDormantAttendees, useArchiveAttendees, useMyFormCapabilities } from '@/hooks/use-forms';
...
export default function AttendeesPage() {
  ...
  const { data, isLoading, isError, error } = useDormantAttendees(undefined, {
    enabled: canSeeAttendees,
  });
    ...
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Dormant-attendee cleanup is restricted to the Admin-department leader, pastors, and admins.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Dormant Attendees</h1>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-10 text-center text-sm text-muted-foreground">
            No dormant attendees. Nothing to archive.
          </CardContent>
        </Card>
      ) : (
        <Card>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Archive {selected.size} attendee(s)?</DialogTitle>
            <DialogDescription>
