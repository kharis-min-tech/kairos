...
export const newBelieverEnrollments = pgTable(
  ...
  (table) => [
    ...
    index('idx_nb_enrollments_mentor_id').on(table.mentorId),
...
export const newBelieverEnrollmentsRelations = relations(newBelieverEnrollments, ({ one, many }) => ({
  member: one(members, { fields: [newBelieverEnrollments.memberId], references: [members.id], relationName: 'studentEnrollments' }),
  branch: one(branches, { fields: [newBelieverEnrollments.branchId], references: [branches.id] }),
  teacher: one(members, { fields: [newBelieverEnrollments.teacherId], references: [members.id], relationName: 'teacherEnrollments' }),
  mentor: one(members, { fields: [newBelieverEnrollments.mentorId], references: [members.id], relationName: 'mentorEnrollments' }),
